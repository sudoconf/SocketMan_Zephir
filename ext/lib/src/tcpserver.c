...
int start_chat(char *ip, int port) {
    ...
    int sock = new_tcp_server(ip, port);
    ev_io listen;
...
void on_readable(struct ev_loop *loop, ev_io *w, int revents) {
    connection_t *c;
    buffer_t *wbuf;
    char buffer[4096];
    int n;
    ...
    } else if (c->recv_bytes > 0 && wbuf->last > wbuf->pos) {
...
void on_writable(struct ev_loop *loop, ev_io *w, int revents) {
    connection_t *c;
    buffer_t *wbuf;
    int sent;
    c = (connection_t *) w->data;
