...
typedef struct {
    ...
    struct ev_loop *loop;
    /* libev loop this thread uses */
    struct ev_async async_watcher;
    /* async watcher for new connect */
...
static void async_cb(EV_P_ ev_async *w, int revents) {
    ...
    if (NULL != item) {
        ...
        printf("thread[%lu] accept: fd :%d  addr:%s port:%d\n",
               ((WORK_THREAD *) (w->data))->thread_id, item->sfd, item->szAddr,
               item->port);
        // cqi_free(item);
        free(item);
        item = NULL;
...
int main(int argc, char **argv) {
    ...
    int listen;
    listen = socket_init();
    ev_io_init(&(dispatcher_thread.accept_watcher), accept_callback, listen,
               EV_READ);
...
void dispath_conn(int anewfd, struct sockaddr_in client) {
    ...
    struct in_addr addr = (struct in_addr) client.sin_addr;
    printf("%d\n", client.sin_port);
    strcpy(lpNewItem->szAddr, (char *) inet_ntoa(client.sin_addr));
    lpNewItem->port = client.sin_port;
    ...
    ev_async_send(work_threads[robin].loop,
                  &(work_threads[robin].async_watcher));
    round_robin++;
}
...
// ev_io write_event;
    ...
    ev_io_start(loop, w);
    printf("thread[%lu] socket fd : %d, turn read 2 write loop! ",
           pthread_self(), fd);
}
