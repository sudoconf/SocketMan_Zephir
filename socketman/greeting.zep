...
%
    ...
    % {
       ...
       static long fibonacci(long n){if (n < 2) return n;
...
public
  static function start(array !descriptor) {
    var ip, port;
    if
      fetch ip, descriptor["ip"] { unset descriptor["ip"]; }
    else {
      let ip = null;
    }
     ...
     */
    if
      fetch port, descriptor["port"] { unset descriptor["port"]; }
    else {
      let port = null;
    }
...
public
  static function fibonacci(int a) {
    print_r(a);
    long result = 0;
    % { result = fibonacci(a); }
    % return result;
...
public
  static function sum(int a, int b) {
    long result = 0;
    % { result = sum(a, b); }
    % return result;
  }
